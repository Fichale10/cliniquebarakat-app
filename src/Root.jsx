...
function Root() {
  ...
  const doLogin = async (email, pw) => {
    ...
    // 1. Chercher dans Supabase — sans RLS ni filtre strict
    if (navigator.onLine && sb) {
      try {
        // Récupérer tous les comptes actifs (évite les problèmes RLS sur filtre email)
        const { data, error } = await sb
          .from('comptes')
          .select('*');

        if (error) {
          console.warn('[Login] Supabase error:', error.message, error.code);
        }

        if (!error && data && data.length > 0) {
          // Sync localStorage avec TOUT Supabase
          setCache('comptes', data);
          setComptes(data);

          // Chercher le compte côté client (insensible à la casse pour l'email)
          const found = data.find(c =>
            (c.email || '').toLowerCase() === emailClean &&
            c.pw === pw &&
            c.actif === true
          );
          if (found) compte = found;
        }
      } catch (e) {
        console.warn('[Login] Supabase exception:', e);
      }
    }
