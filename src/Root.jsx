      ...
      <div style={{width:'100%',maxWidth:'400px',padding:'0 20px'}}>
        ...
        <div style={{textAlign:'center',marginBottom:'18px'}}>
          <div style={{position:'relative',display:'inline-block',marginBottom:'10px'}}>
            ...
            {[[-5,-5],[-5,85],[85,-5],[85,85]].map(([t,l],i)=>(<span key={i} style={{position:'absolute',top:t,left:l,color:'#fbbf24',fontSize:'12px',fontWeight:900}}>★</span>))}
          </div>
        ...
        <div style={{background:'rgba(10,20,35,0.85)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'22px',boxShadow:'0 8px 40px rgba(0,0,0,0.5)',padding:'22px'}}>
          ...
          {ok ? (
            <div style={{textAlign:'center',padding:'20px 0'}}>
              ...
              <p style={{color:'rgba(255,255,255,0.55)',fontSize:'13px',margin:'0 0 16px'}}>Votre demande est en attente d'approbation par l'administrateur. Vous recevrez un accès une fois approuvé.</p>
            ...
            <>
              ...
              <div style={{marginBottom:'12px'}}>
                <label style={{display:'block',fontSize:'10px',fontWeight:800,color:'rgba(255,255,255,0.42)',letterSpacing:'.1em',textTransform:'uppercase',marginBottom:'5px'}}>Nom complet</label>
                <input type="text" value={nom} onChange={e=>setNom(e.target.value)} className="login-dark-input" placeholder="Prénom Nom" onFocus={fi} onBlur={bi}/>
              </div>
              <div style={{marginBottom:'12px'}}>
                <label style={{display:'block',fontSize:'10px',fontWeight:800,color:'rgba(255,255,255,0.42)',letterSpacing:'.1em',textTransform:'uppercase',marginBottom:'5px'}}>Email professionnel</label>
                <input type="email" value={email} onChange={e=>setEmail(e.target.value)} className="login-dark-input" placeholder="votre@email.com" onFocus={fi} onBlur={bi} style={{paddingLeft:'14px'}}/>
              </div>
              <div style={{marginBottom:'12px'}}>
              ...
              <div style={{marginBottom:'12px'}}>
                <label style={{display:'block',fontSize:'10px',fontWeight:800,color:'rgba(255,255,255,0.42)',letterSpacing:'.1em',textTransform:'uppercase',marginBottom:'5px'}}>Mot de passe</label>
                <input type="password" value={pw} onChange={e=>setPw(e.target.value)} className="login-dark-input" placeholder="Minimum 6 caractères" onFocus={fi} onBlur={bi} style={{paddingLeft:'14px'}}/>
              </div>
              <div style={{marginBottom:'18px'}}>
                <label style={{display:'block',fontSize:'10px',fontWeight:800,color:'rgba(255,255,255,0.42)',letterSpacing:'.1em',textTransform:'uppercase',marginBottom:'5px'}}>Confirmer le mot de passe</label>
                <input type="password" value={pw2} onChange={e=>setPw2(e.target.value)} className="login-dark-input" placeholder="Répéter le mot de passe" onFocus={fi} onBlur={bi} style={{paddingLeft:'14px'}}/>
              </div>
...
function ForgotPassword({onBack, onForgot}){
  ...
  const doForgot=async()=>{
    ...
    setLoading(false);
    if(ok) setSent(true);
    else setErr('Impossible d\'envoyer l\'email. Vérifiez votre adresse ou votre connexion.');
  };
      ...
      <div style={{width:'100%',maxWidth:'400px'}}>
        ...
        <div style={{background:'rgba(10,20,35,0.85)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'22px',padding:'22px',boxShadow:'0 8px 40px rgba(0,0,0,0.5)'}}>
          ...
          {sent ? (
            ...
            <>
              {err&&<div style={{background:'rgba(239,68,68,0.14)',border:'1px solid rgba(239,68,68,0.28)',borderRadius:'10px',padding:'10px 13px',marginBottom:'13px',color:'#fca5a5',fontSize:'13px',fontWeight:600}}>⚠️ {err}</div>}
              <p style={{color:'rgba(255,255,255,0.5)',fontSize:'13px',marginBottom:'16px'}}>Entrez votre email et nous vous enverrons un lien de réinitialisation.</p>
...
function Root(){
  ...
  const [comptes,setComptes]=useState(()=>getCache('comptes')||[]);
    ...
    const load=async()=>{
      try{
        if(navigator.onLine&&sb){
          ...
          if(data&&data.length){setComptes(data);setCache('comptes',data);}
        }else{const c=getCache('comptes');if(c&&c.length)setComptes(c);}
      }catch(e){const c=getCache('comptes');if(c)setComptes(c);}
      setAppLoading(false);
    };
    load();
  },[]);
  ...
  const doRegister=async(nom,email,pw,role)=>{
    ...
    const entry={id:newId(),nom,email:email.trim(),pw,role:role||'pharmacien',actif:false,pending:true,created_at:new Date().toISOString()};
    if(navigator.onLine&&sb){
      try{await sb.from('comptes').insert(entry);}catch(e){console.warn(e);}
    }
    const local=[...(getCache('comptes')||comptes),entry];
    setCache('comptes',local);setComptes(local);
    return true;
  };
  ...
  return <App logged={logged} setLogged={(v)=>{if(!v)logAction(user,'deconnexion');setLogged(v);}} user={user} setUser={setUser}/>;
}
