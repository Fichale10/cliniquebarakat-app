...
function MonProfil({ user }) {
  ...
  const changerMDP = async () => {
    ...
    if (pwNew.length < 6) {
      setMsg({ type: 'error', text: 'Minimum 6 caractères.' })
      return
    }
    if (pwNew !== pwConfirm) {
    ...
    if (!user?.email) {
      setMsg({ type: 'error', text: 'Session invalide. Reconnectez-vous.' })
      return
    }
    ...
    try {
      ...
      const { error: updateError } = await sb.auth.updateUser({ password: pwNew })
      ...
      console.warn('[MonProfil] changerMDP:', e)
      setMsg({ type: 'error', text: e.message || 'Impossible de modifier le mot de passe.' })
    } finally {
      setSaving(false)
    }
  }
